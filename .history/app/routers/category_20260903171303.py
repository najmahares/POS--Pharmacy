...
def create_category(
    ...
    """Create a new category. Requires inventory manager or admin role."""
...
def update_category(
    ...
    """Update an existing category. Requires inventory manager or admin role."""
    category = category_repository.get(db, category_id)
    if not category:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    # Check if parent category exists (if provided and not self)
    if data.parent_id:
