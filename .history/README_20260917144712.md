...
| Category  | Hierarchical therapeutic classification (self-referencing tree)           |
...
| DATABASE_URL                 | PostgreSQL connection string    | postgresql://postgres:postgres@localhost:5432/pos |
...
### Why the repository, service, router split?

- Routers handle HTTP concerns only: routing, auth dependencies, status codes
- Services hold business logic such as decrement stock and reject if insufficient
- Repositories isolate SQLAlchemy, making it easy to swap ORM or mock in tests

### Why is user_id Optional in SaleCreate but required on the model?

The router injects user_id from the JWT subject after validation. If the schema required it, the request would fail at Pydantic validation before the router had a chance to override the field, opening the door to client-supplied user_id and impersonation.

### Why atomic stock decrement in the router, not the repository?

The stock check, decrement, and insert must happen in one transaction. Doing it in the router keeps the repository generic while the router coordinates the invariant.

### Why SQLite for tests?

sqlite:// in-memory with StaticPool gives per-test isolation, sub-second startup, and no external dependencies. The production DB is PostgreSQL, using the same SQLAlchemy models and queries.

### Why Decimal everywhere for money?

0.1 + 0.2 is not equal to 0.3 in float. In a POS, that is a class-action lawsuit. Pydantic serializes Decimal to strings so JSON round-trips without precision loss.
...
Built as part of a backend engineering assignment demonstrating FastAPI, SQLAlchemy, JWT authentication, role-based access control, and a rigorous pytest suite.
