...
def test_sale_item_decrements_product_stock(client, auth_headers, product, db_session):
    """After recording a sale of N units, stock must go down by exactly N."""
    ...
    assert product.quantity_in_stock == before - 3, (
        f"Expected {before - 3} but got {product.quantity_in_stock}"
    )


def test_cannot_sell_more_than_stock(client, auth_headers, product, db_session):
    """POS must never allow overselling."""
    sale = _sale(client, auth_headers)
    r = client.post(
        "/sale-items/",
...
def test_total_price_equals_quantity_times_unit(client, auth_headers, product):
    ...
    assert r.status_code == 201
    assert r.json()["total_price"] == "29.97"         


def test_fractional_quantity_for_liquids(client, auth_headers, product):
    """Syrups: sell 0.5 ml? quantity is Decimal(10,3) — must accept .500."""
    sale = _sale(client, auth_headers)
    r = client.post(
        "/sale-items/",
    ...
    assert r.status_code == 201
    assert r.json()["quantity"] in ("0.500", "0.5")
...
def test_rx_product_can_be_sold_with_prescription(client, auth_headers, rx_product):
    ...
    sale_update = client.put(
        f"/sales/{sale['id']}", json=payload, headers=auth_headers
    )
    assert sale_update.status_code == 200
    assert sale_update.json()["prescription_number"] == "RX-12345"
